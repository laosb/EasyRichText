...
//
  ...
  struct ERTTextEditor<RichText: ERTRichText>: NSViewRepresentable {
    @Bindable var editContext: ERTRichTextEditContext<RichText>
    var customize: ((NSTextView) -> Void)?

    init(
      editContext: ERTRichTextEditContext<RichText>,
      customize: ((NSTextView) -> Void)? = nil
    ) {
    ...
    func makeNSView(context: Context) -> NSTextView {
      ...
      editContext.onTextUpdated = { newText in
        ...
        print(
          "ERTTextEditor editContext onTextUpdated text = \(String(describing: textView.textStorage))"
        )
