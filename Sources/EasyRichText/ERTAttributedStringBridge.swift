...
@_spi(Internal) extension NSAttributedString.Key {
  public static let ertSynthesizedItalic = Self(
    "sb.lao.packages.easyrichtext.SynthesizedItalic"
  )
}

public struct ERTAttributedStringBridge: Sendable {
  public static let `default` = ERTAttributedStringBridge()
  ...
  public func nsAttributedString(for attributedString: AttributedString)
    -> NSAttributedString
  {
    let mutableString = NSMutableAttributedString()

    for run in attributedString.runs {
      let mutableSubstring = NSMutableAttributedString(
        AttributedString(attributedString[run.range])
      )
      let range = NSRange(location: 0, length: mutableSubstring.length)

      if bridgeInternalAttributes {
        if run.synthesizedItalic ?? false {
          mutableSubstring.addAttribute(
            .ertSynthesizedItalic,
            value: true,
            range: range
          )
        }
  ...
  public func attributedString(for nsAttributedString: NSAttributedString)
    -> AttributedString
  {
    var attributedString = AttributedString()

    nsAttributedString.enumerateAttributes(
      in: .init(location: 0, length: nsAttributedString.length)
    ) { attributes, range, _ in
      let substring = nsAttributedString.attributedSubstring(from: range)
      var attributedSubstring = AttributedString(substring)

      if bridgeInternalAttributes {
        if let synthesizedItalicValue = attributes[.ertSynthesizedItalic],
          let synthesizedItalic = synthesizedItalicValue as? Bool,
          synthesizedItalic
        {
    ...
    if bridgeColor {
        ...
        for run in attributedString.runs {
          if let nsColor = run.attributes.appKit.foregroundColor {
            attributedString[run.range].swiftUI.foregroundColor = Color(
              nsColor: nsColor
            )
          }
          if let nsColor = run.attributes.appKit.backgroundColor {
            attributedString[run.range].swiftUI.backgroundColor = Color(
              nsColor: nsColor
            )
          }
        }
    ...
    print(
      "ERTAttributedStringBridge attributedString: NSAttributedString: \(nsAttributedString), AttributedString: \(attributedString)"
    )
