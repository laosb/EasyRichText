...
public struct ERTItalicSynthesizer: Sendable {
  ...
  public var fontUtils: ERTFontUtils
    ...
    public init(
      matrix: CGAffineTransform = .init(a: 1, b: 0, c: 0.2, d: 1, tx: 0, ty: 0),
      fontUtils: ERTFontUtils = .default
    ) {
      self.matrix = matrix
      self.fontUtils = fontUtils
    ...
    @_spi(Internal) public func synthesize(_ font: UIFont) -> UIFont {
      fontUtils.font(font) { descriptor in
        let newTraits = font.fontDescriptor.symbolicTraits.subtracting(
          .traitItalic
        )
        let newDescriptor = font.fontDescriptor.withSymbolicTraits(newTraits)?
          .withMatrix(matrix)
  ...
  public func synthesize(_ attributedString: AttributedString)
    -> AttributedString
  {
    ...
    for run in attributedString.runs {
      #if canImport(AppKit)
        guard let font = run.appKit.font,
          font.fontDescriptor.symbolicTraits.contains(.italic)
        else { continue }
      #elseif canImport(UIKit)
        guard let font = run.uiKit.font,
  ...
  public func desynthesize(_ attributedString: AttributedString)
    -> AttributedString
  {
    var attributedString = attributedString

    for run in attributedString.runs {
      guard attributedString[run.range].synthesizedItalic ?? false else {
        continue
      }
