...
public protocol ERTColor: Codable, Hashable, Equatable, Sendable {
...
extension ERTDefinedColor {
  @available(iOS 17.0, macOS 14.0, watchOS 10.0, visionOS 1.0, tvOS 17.0, *)
  private func resolvedColorIsEqual(_ c0: Color.Resolved, _ c1: Color.Resolved)
    -> Bool
  {
  ...
  private func cgColorIsEqual(_ c0: CGColor, _ c1: CGColor) -> Bool {
    guard let components0 = c0.components, let components1 = c1.components
    else { return false }
  ...
  public init(swiftUIColor: Color) {
    ...
    *) {
      ...
      self =
        Self.allCases.first {
          resolvedColorIsEqual($0.swiftUIColor.resolve(in: env), resolvedTarget)
        } ?? .init()
    } else {
      let target = resolveToCGColor(swiftUIColor)
      self =
        Self.allCases.first {
          cgColorIsEqual(resolveToCGColor($0.swiftUIColor), target)
        } ?? .init()
    }
  }
}
